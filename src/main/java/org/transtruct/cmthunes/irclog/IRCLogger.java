...
public class IRCLogger implements IRCChannelListener {
    ...
    public IRCLogger(Connection dbConnection) throws SQLException {
        ...
        this.eventStatement = this.db.prepareStatement("INSERT INTO event_log (event_time, type, user, extra) VALUES (?, ?, ?, ?)");
        this.messageStatement = this.db.prepareStatement("INSERT INTO message_log (msg_time, channel, from_nick, message) VALUES (?, ?, ?, ?)");
    }
    ...
    private void initTables() throws SQLException {
        ...
        statement.addBatch("CREATE TABLE IF NOT EXISTS sessions"
                + " (id INT AUTO_INCREMENT PRIMARY KEY, start_time TIMESTAMP, end_time TIMESTAMP, active BOOLEAN);");
        
        statement.addBatch("CREATE TABLE IF NOT EXISTS event_log"
                + " (id INT AUTO_INCREMENT PRIMARY KEY, event_time TIMESTAMP, type CHAR(8), user VARCHAR(32), extra VARCHAR(64));");
    ...
    private void closeSession(Timestamp timestamp) throws SQLException {
        PreparedStatement statement = this.db.prepareStatement("UPDATE sessions SET active=FALSE, end_time=? WHERE active=TRUE");
        statement.setTimestamp(1, timestamp);
    }
